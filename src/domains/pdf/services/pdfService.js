...
function formatEceTechnicalSummary(line) {
  const details = [
    line.materialName && `Material: ${line.materialName}`,
    line.tintaName && `Tinta: ${line.tintaName}`,
    line.technologyName && `Tecnologia: ${line.technologyName}`,
    line.ai23Status && `AI-23: ${line.ai23Status}`,
  ].filter(Boolean);

  return details.length ? details.join(' | ') : 'Sin referencias tecnicas';
}

function buildEceQuoteHtml(quote) {
  const lines = (quote.lineas || [])
    .map(
      (line) => `
        <tr>
          <td>
            <strong>${escapeHtml(line.descripcion || line.productId || 'Producto')}</strong>
            <small>${escapeHtml(formatEceTechnicalSummary(line))}</small>
          </td>
          <td>${escapeHtml(line.cantidad || 1)}</td>
          <td>${escapeHtml(line.unidad || '')}</td>
          <td>${escapeHtml(line.observaciones || 'Sin observaciones')}</td>
          <td>${escapeHtml(line.subtotal ?? '')}</td>
        </tr>
      `
    )
    .join('');
  ...
  return `
    ...
    <html lang="es">
      <head>
        <meta charset="utf-8" />
        <title>ECE Quote - ${escapeHtml(quote.nombreProyecto || quote.quoteId)}</title>
        <style>
          @page { size: letter; margin: 18mm; }
          * { box-sizing: border-box; }
          body {
      ...
      <body>
        <main>
          <section class="topbar">
            <div class="brand">
              <span>ELANVISUAL</span>
              <strong>ECE Quote</strong>
            </div>
            <div class="meta">
              <p><span>Fecha</span><strong>${escapeHtml(quote.fechaCreacion || '')}</strong></p>
              <p><span>Estado</span><strong>${escapeHtml(quote.estado || '')}</strong></p>
            </div>
          </section>

          <h1>${escapeHtml(quote.nombreProyecto || 'Proyecto')}</h1>

          <section class="summary">
            <div>
              <span>Proyecto</span>
              <strong>${escapeHtml(quote.nombreProyecto || '')}</strong>
            </div>
            <div>
              <span>Cliente</span>
              <strong>${escapeHtml(quote.cliente || '')}</strong>
          ...
          <p class="notice">
            Documento generado desde ECE. PDF solo documenta datos existentes; no calcula ni modifica la cotizacion.
          </p>
        </main>
      </body>
    </html>
...
export const PDFService = Object.freeze({
  ...
  openEceQuotePdf(quote) {
    ...
    pdfWindow.document.open();
    pdfWindow.document.write(buildEceQuoteHtml(quote));
    pdfWindow.document.close();
    pdfWindow.focus();
    pdfWindow.setTimeout(() => pdfWindow.print(), 100);
