...
export default function CotizadorUniversal() {
  ...
  function applyContext(result, { automatic = false } = {}) {
    if (result.customer) setCustomerFromResult(result);
    ...
    setSearchResults([]);
    setSearchError(automatic ? 'Información cargada automáticamente desde CONNECT.' : '');
  }
  ...
  async function runContextSearch(queryOverride, options = {}) {
    ...
    const nextSearchMode = options.customerOnly ? 'customer' : 'design';
    setSearchMode(nextSearchMode);
    setSearching(true);
    setSearchError('');
    try {
      const result = options.customerOnly
        ? await projectContextClient.searchContext(query, { type: 'customer', limit: 12 })
        : await projectContextClient.searchDesignContext(query, { type: type === 'customer' ? 'all' : type, limit: 12 });
      const results = Array.isArray(result.results) ? result.results : [];
      if (options.automatic && results.length === 1) {
        if (nextSearchMode === 'customer') applyCustomerOnly(results[0]);
        else applyContext(results[0], { automatic: true });
      } else {
        setSearchResults(results);
        if (!results.length) {
          setSearchError(nextSearchMode === 'customer'
            ? 'No se encontraron clientes en el CRM con ese dato.'
            : 'No se encontraron solicitudes de Diseño o productos con ese dato.');
        }
      }
    } catch (error) {
      setSearchResults([]);
      setSearchError(error.message || (nextSearchMode === 'customer'
        ...
        : 'No fue posible consultar Diseño y Tienda.'));
    } finally {
      setSearching(false);
    }
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      runContextSearch(query, { type, customerOnly: type === 'customer', automatic: true });
    }, 500);
  ...
  return (
    <main className="uq-shell">
      ...
      <section className="uq-card">
        ...
        <p>Buscá por separado solicitudes de Diseño y productos, o clientes registrados en el CRM de CONNECT.</p>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
          <button type="button" className="uq-light" disabled={searching || searchQuery.trim().length < 2} onClick={() => runContextSearch(undefined, { type: 'all' })}>{searching && searchMode === 'design' ? 'Buscando…' : 'Buscar Diseño / Tienda'}</button>
