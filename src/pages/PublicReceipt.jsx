...
function paymentMethodLabel(value) {
  if (value === 'bank_transfer' || value === 'transfer') {
    return 'Transferencia / depósito bancario';
  }
  if (value === 'electronic_withdrawal') {
...
function readRoute() {
  ...
  const legacy = pathname.replace(/^\/+|\/+$/g, '').toUpperCase();
  return {
    mode: 'legacy',
    accessCode: '',
    receiptCode: '',
...
function quoteUrl(viewUrl, accessCode) {
  if (!viewUrl) return '';
  try {
...
export default function PublicReceipt() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        let receiptNumber = route.receiptNumber;
        let dossierResult = null;

        if (route.mode === 'connected') {
          const portalUrl = new URL('/api/vqs', window.location.origin);
          portalUrl.searchParams.set('path', `public/portal/${route.accessCode}`);
          const portalResponse = await fetch(portalUrl, { cache: 'no-store' });
          const portalPayload = await portalResponse.json().catch(() => ({}));

          if (portalResponse.ok && portalPayload?.data) {
            const portal = portalPayload.data;
            const relatedReceipt = (portal?.receipts || []).find(
              (entry) => String(entry.receiptCode || '') === route.receiptCode
            );

            if (!relatedReceipt) {
              throw new Error('El recibo no pertenece a este portal.');
            }

            const receiptUrl = new URL('/api/vqs', window.location.origin);
            receiptUrl.searchParams.set(
              'path',
              `public/portal/${route.accessCode}/receipts/${route.receiptCode}`
            );
            const receiptResponse = await fetch(receiptUrl, { cache: 'no-store' });
            const receiptPayload = await receiptResponse.json().catch(() => ({}));

            if (!receiptResponse.ok || !receiptPayload?.data) {
              throw new Error(
                receiptPayload?.error?.message ||
                'No fue posible consultar el recibo.'
              );
            }

            const project = (portal?.projects || []).find(
              (item) => String(item.quotationId || '') === String(relatedReceipt.quotationId || '')
            );

            dossierResult = {
              documents: {
                receipts: portal?.receipts || [],
                quotation: project?.viewUrl
                  ? { viewUrl: quoteUrl(project.viewUrl, route.accessCode) }
                  : null,
                workOrder: project?.workOrder || null
              }
            };

            if (active) {
              setDossier(dossierResult);
              setReceipt(receiptPayload.data);
            }
            return;
          }

          dossierResult = await getPublicCustomerDossier(route.accessCode);
          const relatedReceipt = (dossierResult?.documents?.receipts || []).find(
            (entry) => String(entry.receiptCode || '') === route.receiptCode
          );

          if (!relatedReceipt) {
            throw new Error('El recibo no pertenece a este expediente.');
          }

          receiptNumber = relatedReceipt.receiptNumber;
          if (active) setDossier(dossierResult);
        }

        if (!/^ELV-REC-\d{4}-\d{6}$/.test(String(receiptNumber || '').toUpperCase())) {
          throw new Error('Número de recibo ELANVISUAL no válido.');
        }

        const response = await fetch(
          `${CONNECT_BASE_URL}/api/v1/business/vqs/public/receipts/${encodeURIComponent(receiptNumber)}`,
          { cache: 'no-store' }
        );
        ...
        if (
          ...
          String(payload.data.platform).toUpperCase() !== 'ELANVISUAL'
        ) {
        ...
        if (active) {
          setError(cause?.message || 'No fue posible consultar el recibo.');
        }
      } finally {
  ...
  if (loading) {
    return (
      <main className="public-receipt-shell">
        <div className="receipt-status">Cargando recibo…</div>
      </main>
    );
  }
  ...
  if (error || !receipt) {
    return (
      <main className="public-receipt-shell">
        <div className="receipt-status receipt-error">
          <strong>Recibo no disponible</strong>
          <span>{error || 'No encontramos este recibo.'}</span>
        </div>
      </main>
    );
  }
  ...
  const isPaid =
    Number(receipt.pendingBalanceUsd || 0) <= 0.009 &&
    Number(receipt.quotationTotalUsd || 0) > 0;
  ...
  const relatedReceipts = dossier?.documents?.receipts || [];
  ...
  return (
    <main className="public-receipt-shell">
      <section className="public-receipt-card">
        ...
        <div className="receipt-body">
          <div className="receipt-project">
            ...
            <strong>{receipt.projectName || 'Proyecto ELANVISUAL'}</strong>
          </div>
          ...
          <div className="receipt-grid">
            ...
            <Row
              label="Fecha de pago"
              value={new Date(receipt.paidAt).toLocaleString('es-NI')}
            />
          ...
          <div className="receipt-grid">
            ...
            <Row
              label={isWithdrawal ? 'Referencia / operación' : 'Referencia bancaria'}
              value={receipt.paymentReference}
            />
            {isNio && (
              <Row label="Monto recibido" value={money(receipt.originalAmount, 'NIO')} strong />
            )}
            {isNio && (
              <Row
                label="Tipo de cambio"
                value={`C$ ${Number(receipt.exchangeRate || 0).toFixed(4)} / USD`}
              />
            )}
            <Row
              label={isNio ? 'Monto aplicado' : 'Monto recibido'}
              value={money(receipt.amountUsd, 'USD')}
              strong
            />
          </div>
          ...
          <div className="receipt-grid receipt-financial">
            ...
            <Row
              ...
              value={money(receipt.pendingBalanceUsd)}
              strong
            />
          </div>
        ...
        <footer className="receipt-footer">
          ...
          <div className="receipt-contact">
            <a href={PLATFORM_URL}>visual.elankav.com</a>
            <span>•</span>
      ...
      <nav className="receipt-actions no-print">
        <button type="button" onClick={() => window.print()}>Descargar PDF</button>
        {dossier?.documents?.quotation?.viewUrl && (
          <a href={dossier.documents.quotation.viewUrl}>Cotización</a>
        )}
        {dossier?.documents?.workOrder?.viewUrl && (
          <a href={dossier.documents.workOrder.viewUrl}>Seguimiento OT</a>
        )}
        {!dossier && <a href={PLATFORM_URL}>Ir a ELANVISUAL</a>}
      </nav>

      {relatedReceipts.length > 1 && (
        <nav className="receipt-related no-print">
          <strong>Otros recibos del proyecto</strong>
          {relatedReceipts
            .filter((entry) => entry.receiptNumber !== receipt.receiptNumber)
            .map((entry) => (
              <a key={entry.receiptNumber} href={entry.viewUrl}>
                {entry.receiptNumber}
              </a>
            ))}
        </nav>
