...
export default function ELANCopilotPanel() {
  ...
  async function manejarArchivos(e){ const lista=Array.from(e.target.files||[]); if(!lista.length)return; setError(""); setEstado("Leyendo archivos..."); try{ setArchivos((prev)=>[...prev,...await prepararArchivosTemporalesAI(lista)]); setEstado(""); }catch(err){ setError(err.message||"No se pudieron preparar los archivos."); } e.target.value=""; }
