...
export default function PortalProduccionCRM() {
  ...
  return <div style={{ padding: 20 }}><h2>Portal Producción</h2><p style={{ color: '#6b7280' }}>Panel operativo para taller, instaladores y supervisores.</p><div style={grid}><div style={tarjeta}><strong>Órdenes</strong><h3>{ordenesTrabajo.length}</h3></div><div style={tarjeta}><strong>Producciones</strong><h3>{produccion.length}</h3></div><div style={tarjeta}><strong>Materiales</strong><h3>{materiales.length}</h3></div><div style={tarjeta}><strong>Inventario</strong><h3>{inventario.length}</h3></div></div></div>;
}
