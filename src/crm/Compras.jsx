...
export default function Compras() {
  ...
  const cambiar = (e) => {
    ...
    setForm((prev) => {
      ...
      if (name === 'tipoFiscal') {
        nuevo.facturaFiscal = value === 'Con IVA' ? 'Si' : 'No';
      }
  ...
  const editar = (item) => {
    ...
    setForm({
      ...
      facturaFiscal: item.facturaFiscal || (numero(item.iva) > 0 ? 'Si' : 'No'),
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        ...
        <p style={styles.subtitle}>Compras con control fiscal: IVA credito, compras sin factura y gasto interno real.</p>
      </div>
      ...
      <form style={styles.card} onSubmit={guardar}>
        <div style={styles.grid}>
          ...
          <label style={styles.label}>Categoria<select style={styles.select} name="categoria" value={form.categoria} onChange={cambiar}>{CATEGORIAS.map((c) => <option key={c} value={c}>{c}</option>)}</select></label>
        </div>
