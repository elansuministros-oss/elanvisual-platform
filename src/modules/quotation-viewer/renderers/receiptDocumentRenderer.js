...
function blobToDataUrl(blob, FileReaderClass = globalThis.FileReader) {
  return new Promise((resolve, reject) => {
    if (!FileReaderClass) {
      reject(new Error('FileReader no disponible para incrustar el logo.'));
      return;
    }
    const reader = new FileReaderClass();
    reader.onload = () => resolve(text(reader.result));
    reader.onerror = () => reject(new Error('No fue posible convertir el logo para impresión.'));
...
export async function loadEmbeddedReceiptLogo(quotation = {}, options = {}) {
  ...
  if (!brand.logoUrl) return { brand, logoDataUrl: '' };
  ...
  const logoDataUrl = await blobToDataUrl(blob, options.FileReaderClass);
  if (!logoDataUrl.startsWith('data:image/')) throw new Error('El logo recibido no pudo incrustarse como imagen.');
...
export function buildReceiptDocument(payment = {}, quotation = {}, options = {}) {
  ...
  const logoDataUrl = text(options.logoDataUrl);
  const logo = logoDataUrl
    ? `<span class="brand-logo-box is-dark"><img class="brand-logo" src="${safe(logoDataUrl)}" alt="${safe(brand.name)}"></span>`
...
export async function printReceiptDocument(payment, quotation, windowObject = window) {
  ...
  try {
    const embedded = await loadEmbeddedReceiptLogo(quotation, {
      ...
      FileReaderClass: windowObject.FileReader || globalThis.FileReader
    });
