...
function absoluteAssetUrl(value, baseUrl = '') {
  ...
  try {
    return new URL(assetUrl, `${text(baseUrl).replace(/\/+$/, '')}/`).href;
  } catch {
    return assetUrl;
  }
}
...
function receiptBrand(quotation = {}, baseUrl = '') {
  ...
  const logoUrl = text(
    brand.logoForLightBackground
    || brand.logo_for_light_background
    || brand.logoLightUrl
    || brand.logo_light_url
    || brand.logoUrl
    || brand.logo_url
    || QUOTATION_FALLBACK_LOGO_URL
  );
...
export function buildReceiptDocument(payment = {}, quotation = {}, options = {}) {
  ...
  const logo = brand.logoUrl
    ? `<span class="brand-logo-box is-dark"><img class="brand-logo" src="${safe(brand.logoUrl)}" alt="${safe(brand.name)}"></span>`
