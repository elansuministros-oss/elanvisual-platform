...
async function handleChat(payload = {}) {
  ...
  if (!apiKey) {
    return {
      ok: false,
      error: 'OPENAI_API_KEY no configurada en ELANVISUAL.'
    };
  }

  const mensaje = String(
    payload.mensaje || payload.message || payload.prompt || ''
  ).trim();
      ...
      input: [
        {
          role: 'system',
          content:
            'Eres ELAN AI, asistente operativo de ELANKAV. Responde de forma clara, comercial y útil. No reveles lógica interna de costos ni fórmulas privadas.'
        },
        {
          role: 'user',
          content: mensaje
